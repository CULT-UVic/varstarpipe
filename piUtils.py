...
def readSpe(spefilename, verbose=False):
  ...
  timeLocal = struct.unpack_from("6s", header, offset=172)[0]

  timeUTC = struct.unpack_from("6s", header, offset=179)[0]
...
def speDictToFits(spedict, outfile=None, clobber=True, verbose=False):
  """
  ...
  #for frameNumber in range( len(spedict['data']) ):
    ...
    for kk in hdrDesc.keys():
      hdu.header.set(kk, spedict[kk], hdrDesc[kk])
